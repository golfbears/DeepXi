...
def framesig(sig, preemph, frame_len, frame_step, winfunc=lambda x: numpy.ones((x,)), stride_trick=True):
    ...
    res = numpy.asarray(preemphSignal).flatten()
    indices = numpy.tile(numpy.arange(0, frame_len), (numframes, 1)) \
              + numpy.tile(numpy.arange(0, numframes * frame_len, frame_len), (frame_len, 1)).T
